...
2. **각 도구별 상세 정보**: 각 도구마다 다음 정보를 **모두** 포함하세요:
   ...
   - **리뷰 기능 지원 여부**: 코드 리뷰 요구사항이 있으면, 이 도구가 리뷰 기능을 지원하는지 명시
...
- 🚨🚨🚨 **내부 분석 결과 활용 (가장 먼저 확인하세요!)** 🚨🚨🚨
  * **🚨 절대 규칙 1**: {constraints} 섹션에 "내부 분석 결과"가 있으면:
    ...
    - 🚨 **코드 리뷰 요구사항 반영 (매우 중요!)**: 사용자가 "코드 작성과 리뷰"를 요청했다면:
      ...
      * 리뷰 기능을 지원하면: "이 도구는 코드 작성과 리뷰 기능을 모두 지원합니다"라고 명시하세요!
      * 리뷰 기능을 지원하지 않으면: 반드시 "이 도구는 코드 작성에 특화되어 있으며, 코드 리뷰 기능이 필요하다면 Findings에서 확인한 PR 리뷰 전용 도구와 함께 사용하는 것을 권장합니다"라고 조합을 제안하세요! (⚠️ 도구명은 Findings에서 확인한 실제 도구명만 사용하세요!)
      * 리뷰 도구가 추천되지 않았다면, Findings에서 확인한 리뷰 전용 도구를 명시적으로 언급하고 조합 사용을 제안하세요!

...
1. **출처는 연구 결과(Findings)에 있는 URL만 사용!**
   - 연구 결과를 자세히 읽고 실제로 나온 URL만 출처에 포함
   - 추측으로 URL 만들기 절대 금지! (예: https://cursor.so/pricing 같은 추측 URL 금지!)
   - 연구 결과에 URL이 없으면 출처 섹션을 작성하지 마세요
   
2. **정보는 연구 결과(Findings)에서만 가져오기**
   - 가격, 기능, IDE 지원 등 모든 정보는 연구 결과에서만
   - 연구 결과에 없는 정보는 작성하지 마세요
   - 추측 금지! 확인된 정보만!

3. **언어 및 날짜**
   - 사용자 메시지가 한국어면 반드시 한국어로 작성!
   - **December 2025 기준 최신 정보**로 작성!
   - 2024년 이전 정보는 절대 사용 금지!
...
⚠️ **중요**: 연구 결과(Findings)에 실제로 나온 URL만 포함하세요!
추측 URL 절대 금지!
...
1. **연구 결과(Findings)에 있는 URL만 사용**
   - 연구 결과를 자세히 읽고 실제로 나온 URL만 출처에 포함
   - 연구 결과에 없는 URL은 절대 작성 금지!
   
2. **출처 작성 전 반드시 확인**
   - ✅ 이 URL이 연구 결과(Findings)에 있는가?
   - ✅ 이 제목이 연구 결과에서 나온 것인가?
   - ❌ 추측으로 URL 만들기 절대 금지!
   - ❌ "아마도 이 사이트일 것 같다" 금지!

3. **출처 없으면 출처 섹션 생략**
   - 연구 결과에 URL이 없으면 출처 섹션을 작성하지 마세요
   - 빈 출처 목록보다 출처 섹션 없는 것이 낫습니다

4. **잘못된 출처 예시 (절대 금지!)**
   ```
   ❌ [1] GitHub Copilot Pricing - https://github.com/features/copilot/pricing
   ❌ [2] Cursor AI Pricing - https://cursor.so/pricing
   (연구 결과에 없는 추측 URL!)
   ```

5. **올바른 출처 예시**
   ```
   ✅ [1] GitHub Copilot Features - https://docs.github.com/copilot
   (연구 결과에 실제로 나온 URL)
   ```

**최종 검증:**
- 출처 섹션을 작성하기 전에 다시 한번 연구 결과(Findings)를 확인하세요
- 각 출처가 연구 결과에 실제로 존재하는지 재확인하세요
- 의심스러우면 출처를 포함하지 마세요!
</🚨🚨🚨 출처 규칙>

...
1. **모든 추천 도구 포함 필수**: 내부 분석 결과의 모든 추천 도구(최대 3개)를 반드시 포함하세요!
2. **각 도구별 상세 정보**: 각 도구마다 다음 정보를 포함하세요:
   - **💰 가격**: 팀 규모 기준 계산된 실제 비용 (내부 분석 결과의 비용 정보 사용)
   - **🔗 통합 기능**: 주요 통합 서비스 목록
   - **✅ 장점**: 왜 이 도구가 추천되는지 구체적인 이유
   - **추천 이유**: 내부 분석 결과의 판단 근거를 자연스럽게 변환
3. **리포트 길이**: 최소 1000자 이상, 모든 추천 도구를 상세히 설명
4. **결론 섹션**: 모든 추천 도구를 요약하고 조합 사용을 제안 (여러 도구 추천 시)
...
- 🚨🚨🚨 **내부 분석 결과 활용 (가장 먼저 확인하세요!)** 🚨🚨🚨
  * **🚨 절대 규칙 1**: {constraints} 섹션에 "내부 분석 결과"가 있으면:
    ...
    - 🚨 **코드 리뷰 요구사항 반영**: 사용자가 "코드 작성과 리뷰"를 요청했다면, 내부 분석 결과의 추천 도구가 리뷰 기능을 지원하는지 자연스럽게 언급하세요! 리뷰 기능이 없다면 조합(코드 작성 AI + PR 리뷰 AI)을 제안하세요!
    ...
    - 🚨 **리포트 완성도 필수**: 내부 분석 결과의 모든 추천 도구를 반드시 포함하세요! 각 도구에 대해 가격, 통합 기능, 장점, 추천 이유를 상세히 설명하세요! 리포트는 최소 1000자 이상이어야 합니다!
...
- 🚨 **코드 리뷰 요구사항 반영 (매우 중요!)**:
  * 사용자가 "코드 작성과 리뷰", "코드 리뷰", "PR 리뷰"를 명시했다면:
    ...
    - 단일 도구가 리뷰 기능을 지원하면: "이 도구는 코드 작성과 리뷰 기능을 모두 지원합니다"라고 명시하세요!
    - 단일 도구가 리뷰 기능을 지원하지 않으면: 조합을 제안하세요 (코드 작성 AI + PR 리뷰 AI)
    - 리뷰 기능 지원 여부를 답변에 반드시 포함하세요!
- 🚨 **선택 질문 처리 (매우 중요!)**:
...
1. **[도구명]** - [이유]
   - 🚨 **주의**: 같은 기능을 하는 다른 도구와 함께 추천하지 마세요!
   - 하나로 충분하면 하나만 추천하세요!
   - 🚨 **코드 리뷰 요구사항 확인**: 사용자가 "코드 작성과 리뷰"를 요청했다면, 이 도구가 리뷰 기능을 지원하는지 반드시 확인하고 언급하세요!
   - 예: "[도구명1] 하나만으로도 코드 작성의 70~80%는 커버됩니다. 여기에 [도구명2]를 추가해도 얻는 이익은 거의 없습니다."
   - 예 (코드 리뷰 요구 시): "[도구명1]은 코드 작성과 리뷰 기능을 모두 지원합니다" 또는 "[도구명1]은 코드 작성만 지원하므로, 리뷰 기능이 필요하다면 [PR 리뷰 전용 도구명]와 조합하는 것을 권장합니다"

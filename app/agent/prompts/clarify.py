...
- **예시 (질문 내용에 따라 다르게 생성):**
  ...
  * "표로 정리해줘" → "네! 표 형식으로 정리해드리겠습니다." (🚨 매우 중요: 표 형식 요청!)
  * "표로 만들어줘" → "네! 표로 만들어드리겠습니다."
  * "1순위 추천해줘" → "네! 조건에 맞는 1순위를 추천드리겠습니다."
  * "비교해줘" → "네! 상세히 비교 분석해드리겠습니다."
  * "왜 추천했어?" → "네! 추천 이유를 설명해드리겠습니다."
  * "설정 방법 알려줘" → "네! 설정 방법을 안내해드리겠습니다."
- ❌ **금지**: 고정된 멘트 사용 금지 (예: "네! 조건에 맞춰 분석해드리겠습니다." 같은 반복 멘트)
- ❌ **금지**: 모든 질문에 동일한 멘트 사용 금지
- ✅ **원칙**: 질문의 핵심 내용(표, 비교, 가격 등)을 반드시 반영하여 다르게 생성
...
**🔍 3단계: 검색 필요 여부 판단 (Follow-up 질문인 경우만!)**

**🚨 매우 중요: is_followup = YES인 경우에만 이 단계를 적용하세요!**

**🚨 핵심 원칙: 사용자 질문의 의도를 정확히 파악하세요!**
- 단순히 "표로 정리해줘"만 요청하는지?
- 아니면 추가 정보를 요청하면서 "표로 정리해줘"를 함께 요청하는지?
- 이전 대화에 필요한 정보가 모두 있는지?

**📋 검색 불필요한 경우 (need_research = false) - 다음 조건을 모두 만족해야 함:**

**조건 1: 형식 변환만 요청 (추가 정보 요청 없음)**
- 단순 형식 변환 요청만 있는 경우
  * "표로 정리해줘", "표로 만들어줘", "표로", "리스트로", "표 형식으로", "표 형식"
  * "방금 추천해준거 표로 정리해줘", "아까 추천한 것 표로"
  * "지금 추천해준거 표로", "이전 추천 표로"
  * **중요**: 추가 정보 요청 키워드가 없어야 함!
  * **🚨 매우 중요**: "표로 정리해줘", "표로 만들어줘" 같은 요청은 거의 항상 형식 변환만 요청! 이전 대화에서 추천한 도구를 표로 정리하는 것이 목적!

**조건 2: 이전 대화에서 필요한 정보가 모두 있음**
- 이전 대화에서 이미 추천한 도구 목록이 있음
- 이전 대화에서 이미 언급한 정보(가격, 장점, 단점 등)가 있음
- 사용자가 요청한 정보가 이전 대화에 모두 포함되어 있음

**✅ 검색 불필요 예시 (need_research = false):**
- "표로 정리해줘" → 형식 변환만, 이전 대화 정보만 필요 (🚨 매우 중요!)
- "표로 만들어줘" → 형식 변환만, 이전 대화 정보만 필요
- "방금 추천해준거 표로 정리해줘" → 형식 변환만, 이전 대화 정보만 필요
- "아까 추천한 것 리스트로" → 형식 변환만, 이전 대화 정보만 필요
- "지금 추천해준거 표로" → 형식 변환만, 이전 대화 정보만 필요
- "이전 추천 표로" → 형식 변환만, 이전 대화 정보만 필요
- "1순위는 뭔가요?" (이전 대화에 1순위 명시되어 있으면) → 이전 대화 참조만 필요
- "가격은?" (이전 대화에 가격 명시되어 있으면) → 이전 대화 참조만 필요

**🚨 매우 중요: "표로 정리해줘", "표로 만들어줘" 같은 요청은 거의 항상 need_research = false입니다!**
- 이 요청들은 이전 대화에서 추천한 도구를 표 형식으로 재정리하는 것이 목적입니다
- 새로운 정보를 수집하거나 새로운 도구를 찾는 요청이 아닙니다
- 단순히 표시 형식만 변경하는 요청이므로 재검색이 불필요합니다

**📋 검색 필요한 경우 (need_research = true) - 다음 중 하나라도 해당하면:**

**조건 1: 추가 정보를 요청하면서 형식 변환도 함께 요청**
- "방금 추천해준거 가격도 포함해서 표로 정리해줘" → 가격 정보 추가 필요, 검색 필요!
- "방금 추천해준거 상세 정보 표로 정리해줘" → 상세 정보 추가 필요, 검색 필요!
- "방금 추천해준거 비교해서 표로 정리해줘" → 비교 정보 추가 필요, 검색 필요!
- "방금 추천해준거 통합 기능까지 표로 정리해줘" → 통합 기능 추가 필요, 검색 필요!

**조건 2: 새로운 정보 요청 (형식 변환 요청 없음)**
- "추가로 A 도구는 어떤가요?", "B 도구 비교해줘" → 새로운 도구 조사 필요, 검색 필요!
- "설정 방법 알려줘", "도입 가이드 알려줘" (이전 대화에 없으면) → 추가 정보 수집 필요, 검색 필요!
- "가격은?" (이전 대화에 가격 정보가 없으면) → 가격 정보 수집 필요, 검색 필요!

**조건 3: 이전 대화에 필요한 정보가 없음**
- 사용자가 요청한 정보(예: 가격, 상세 정보, 비교 등)가 이전 대화에 없음
- 이전 대화에 도구명만 있고 상세 정보가 없음

**조건 4: 처음 질문인 경우 (is_followup = NO)**
- 항상 검색 필요 (need_research = true)

**✅ 검색 필요 예시 (need_research = true):**
- "방금 추천해준거 가격도 포함해서 표로 정리해줘" → 가격 정보 추가 필요
- "방금 추천해준거 상세 정보 표로 정리해줘" → 상세 정보 추가 필요
- "방금 추천해준거 비교해서 표로 정리해줘" → 비교 정보 추가 필요
- "가격은?" (이전 대화에 가격 정보가 없으면) → 가격 정보 수집 필요
- "추가로 Cursor는 어떤가요?" → 새로운 도구 조사 필요

**🚨 판단 기준 (매우 중요!):**
1. **사용자 질문을 자세히 읽으세요!**
   - "표로 정리해줘"만 있는가? → 검색 불필요 가능성 높음
   - "가격도 포함해서", "상세 정보", "비교해서" 같은 추가 요청이 있는가? → 검색 필요!
   
2. **이전 대화({messages})에 필요한 정보가 모두 있는지 확인하세요!**
   - 사용자가 요청한 정보가 이전 대화에 모두 있으면 → 검색 불필요
   - 사용자가 요청한 정보가 이전 대화에 없으면 → 검색 필요
   
3. **의도를 파악하세요!**
   - 단순 정리/변환 요청? → 검색 불필요 가능
   - 추가 정보 요청? → 검색 필요

**🔍 4단계: Verification 메시지 작성**
...
{{
  ...
  "need_research": true/false (is_followup = YES인 경우만 판단, 처음 질문이면 항상 true),

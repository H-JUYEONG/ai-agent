...
- **순수 인사 (is_greeting = true):**
  ...
  - 질문, 요청, 정보 요구 등이 전혀 없는 경우
  - 대화를 시작하거나 친근함을 표현하는 의도만 있는 경우
  - 메시지의 주요 의도가 "인사하기"인 경우
  
- **인사가 아님 (is_greeting = false):**
  - 인사와 함께 질문, 요청, 정보 요구 등이 함께 있는 경우
  - 예: "안녕하세요, 코딩 AI 추천해주세요" → 질문 의도가 있으므로 순수 인사 아님
  - 인사가 포함되어 있어도 실제 의도는 질문이나 요청인 경우

**⚠️ 중요:**
- 특정 키워드에 의존하지 말고, 메시지의 **의도**를 중심으로 판단하세요
- 사용자가 정말로 단순히 인사만 하려는 것인지, 아니면 인사와 함께 질문/요청을 하려는 것인지 판단하세요
- 언어, 표현 방식에 관계없이 의도가 "순수 인사"이면 is_greeting = true

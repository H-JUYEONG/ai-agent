...
- ❌ 데이터베이스 도구 추천 금지
- ❌ 일반 개발 도구 추천 금지
- ❌ **코드 품질 분석 도구 (SonarQube, CodeClimate, Codacy 등) 추천 금지!** 이런 도구들은 AI 코딩 도구가 아닙니다!
- ✅ **코딩 AI 어시스턴트/자동완성 도구만** 추천 (⚠️ 위 "Cursor, GitHub Copilot, Codeium" 등은 예시일 뿐, 실제 검색 결과에서 확인한 도구만 사용!)
- ✅ **AI 기반 코드 생성/자동완성 도구만** 검색하세요 (예: GitHub Copilot, Cursor, Codeium, Tabnine, Aider 등)

**Follow-up (is_followup = YES):**
- 🚨🚨🚨 **매우 중요: 이전 추천 도구만 사용!** 🚨🚨🚨
- 이전 추천: {previous_tools}
- **절대 규칙 1: {previous_tools}에 도구명이 있으면, 오직 이 도구들만 연구하세요! 새로운 도구를 찾거나 검색하는 것은 절대 금지!**
- **절대 규칙 2: 사용자 질문에 "아까 추천 중에", "방금 추천받은 것", "방금 추천해준거", "지금 추천해준거", "방금 추천받은 구성", "그러면 아까 추천 중에" 같은 표현이 있으면, 반드시 이전 대화({messages})에서 추천한 도구만 사용하세요!**
- **절대 규칙 3: 사용자가 요청한 정보(가격, 기능, 사용 방법 등)만 수집하세요. 이전 도구에 대한 추가 정보만 수집하고, 새로운 도구를 찾거나 검색하지 마세요!**
- 🚨 **금지**: 새로운 도구를 찾거나 검색하는 것은 절대 금지!
- 🚨 **금지**: "1순위", "어떤 게", "추천", "추천해줘" 같은 질문은 이전 추천에서 명확하게 답해야 함!
- 🚨 **금지**: {previous_tools}에 나열된 도구 외의 다른 도구를 찾거나 추천하는 것은 절대 금지!

**Follow-up 질문 유형별 처리:**
- **"1순위는 뭔가요?"** → 이전 대화({messages})에서 1순위로 추천한 도구를 명확히 답변 (새로운 도구 검색 금지!)
- **"어떤 게 좋을까요?"** → 이전 대화({messages})에서 추천한 도구를 참조하여 명확히 답변 (새로운 도구 검색 금지!)
- **"추천해줘"** (Follow-up) → 이전 대화({messages})에서 추천한 도구를 참조하여 답변 (새로운 도구 검색 금지!)
- **"방금 추천받은 구성", "아까 추천받은 것", "지금 추천해준거", "지금 추천해준 것"** → 이전 대화({messages})에서 추천한 도구만 사용 (새로운 도구 검색 금지!)
- **"표로 정리해줘", "표로 만들어줘"** (Follow-up) → 이전 대화({messages})에서 추천한 도구만 표 형식으로 정리 (새로운 도구 검색 금지!)
- **정보 요청** (가격, 기능 등) → "{previous_tools}의 [요청 정보]를 조사하세요. (새로운 도구 검색 금지!)"
...
**연구 질문 템플릿 (⚠️ 위 도구명은 예시일 뿐, {previous_tools}에 나열된 실제 도구명 사용!):**
- **"1순위", "어떤 게", "추천" 질문**: "{previous_tools} 중에서 이전 대화에서 1순위로 추천한 도구를 명확히 제시하세요. 이전 대화({messages})의 추천 순서를 그대로 따르세요. (새로운 도구 검색 금지!)"
- **"방금 추천받은 구성" 질문**: "{previous_tools}를 [사용자 조건]에 맞춰 평가하세요. 이전 대화({messages})에서 추천한 순서를 유지하세요. (새로운 도구 검색 금지!)"
- **정보 요청**: "{previous_tools}의 [요청 정보]를 조사하세요. (새로운 도구 검색 금지!)"
- **의사결정**: "{previous_tools}를 [사용자 조건: 예산, 보안, 팀 규모]에 맞춰 평가하고 [개수]를 추천하세요. 이전 대화({messages})의 추천 순서를 그대로 따르세요. (새로운 도구 검색 금지!)"
